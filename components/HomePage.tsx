...
const HomePage: React.FC = () => {
  ...
  const quizForTodayData = tousLesQuestionnaires[today];
        ...
        {quizForTodayData ? (
          <Link to={`/quiz/${today}`}>
            <Button variant="primary" className="w-full text-lg">
              ...
              {quizForTodayData.theme ? ` - ${quizForTodayData.theme}` : ''}
            </Button>
          </Link>
        ) : (
        ...
        <section>
          ...
          <ul className="space-y-3">
            {pastQuizDates.map(date => {
              const quizData = tousLesQuestionnaires[date];
              return (
